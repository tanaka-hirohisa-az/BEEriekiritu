...
SELECT
  ...
  NVL(RTRIM(TO_CHAR(t.F_協定単価,'FM999999999999999999999999999990.999999'),'.'),' ') || '<|>' ||
  NVL(RTRIM(TO_CHAR(t.F_材料単価,'FM999999999999999999999999999990.999999'),'.'),' ') || '<|>' ||
  NVL(RTRIM(TO_CHAR(t.F_加工単価,'FM999999999999999999999999999990.999999'),'.'),' ') || '<|>' ||
  NVL(RTRIM(TO_CHAR(t.F_管理単価,'FM999999999999999999999999999990.999999'),'.'),' ') || '<|>' ||
...
WHERE MOD(ORA_HASH(t.F_納入品目番号||'#'||t.F_得意先番号||'#'||t.F_開始日), 4) = 0;